/**
 * This class is stolen from Princeton University, you can find the original
 * implementation at the link below:
 * https://algs4.cs.princeton.edu/14analysis/PolynomialRegression.java.html
 * 
 * Changed the name from PolynomialRegression --> TorquePolyRegression 
 * 
 * The regression solved linear least squares by performing QR decomposition
 * on a matrix. 
 * 
 * The class uses Jama, a library from the National Institute of Standards 
 * and Technology (https://nist.gov) to handle matrix computation. This must
 * be added as a dependency to the build.gradle file by adding the following 
 * line inside the dependencies section:
 * implementation group: 'gov.nist.math', name: 'jama', version: '1.0.3'
 * 
 * It also came to my attention later on that FRC 254 stole the class in 2020,
 * and then FRC 1678 stole it from them in 2022. So I guess we are stealing it
 * from them in 2024, but indirectly.
 * 
 * -Jus
 */
...
/**
 * The {@code TorquePolyRegression} class performs a polynomial regression
 ...
 * @author Kevin Wayne
 */
public class TorquePolyRegression implements Comparable<TorquePolyRegression> {
     ...
     *                                  equal
     */
    public TorquePolyRegression(double[] x, double[] y, int degree) {
        this(x, y, degree, "n");
    }
     ...
     *                                  equal
     */
    public TorquePolyRegression(double[] x, double[] y, int degree, String variableName) {
        this.degree = degree;
     ...
     */
    public int compareTo(TorquePolyRegression that) {
        double EPSILON = 1E-5;
    ...
    /**
     * Unit tests the {@code TorquePolyRegression} data type.
     *
    ...
    public static void main(String[] args) {
        ...
        TorquePolyRegression regression = new TorquePolyRegression(x, y, 3);
