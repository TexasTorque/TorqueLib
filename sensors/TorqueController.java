...
public final class TorqueController {
     ...
     */
    public TorqueController(final int port) {
     ...
     * @param port The controller port.
     */
    public TorqueController(final ControllerPort port) {
        this(port.ordinal());
    }
     ...
     * @param deadband The deadband.
     */
    public TorqueController(final ControllerPort port, final double deadband) {
        this(port.ordinal(), deadband);
    }
    ...
    private final double scale(final double raw) {
        return TorqueMath.toleranced(raw, 0) ? 0 
                : (raw - Math.signum(raw) * deadband) / (1 - deadband);
    }

    // * Low level Joystick calls

    private final double axis(final int channel) {
        return stick.getRawAxis(channel);
    }

    private final boolean down(final int channel) {
        return stick.getRawButton(channel);
    }

    private final boolean pressed(final int channel) {
        return stick.getRawButtonPressed(channel);
    }

    private final boolean released(final int channel) {
        return stick.getRawButtonReleased(channel);
    }
    ...
    public final boolean isLeftTriggerDown() {
        return getLeftTriggerAxis() >= TRIGGER_DOWN_POSITION;
    }
    ...
    public final boolean isRightTriggerDown() {
        return getRightTriggerAxis() >= TRIGGER_DOWN_POSITION;
    }
    ...
    public static enum DPADState {
        ...
        public static final DPADState fromAngle(final int angle) {
            return values()[angle];
        }
    }
     ...
     */
    public final DPADState getDPADState() { 
        return DPADState.fromAngle(stick.getPOV()); 
    }
     ...
     */
    public final boolean isDPAD(final int angle) {
        return getDPADState().getAngle() == angle; 
    } 
     ...
     */
    public final boolean isDPAD(final DPADState state) { 
        return getDPADState() == state; 
    } 
